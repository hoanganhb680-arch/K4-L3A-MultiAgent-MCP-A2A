"""Write the updated workflow.py with correct MCP tool names."""
...
def _verifier(
    ...
    payment_refs = [str(p.get("payment_sequential", i)) for i, p in enumerate(payments)]
...
with open("src/student_agent/workflow.py", "w", encoding="utf-8") as f:
    f.write(WORKFLOW)

ast.parse(WORKFLOW)
print(f"Written OK — {WORKFLOW.count(chr(10))} lines, syntax valid")

# Quick sanity check: correct tool names present
assert "get_order_items" in WORKFLOW, "MISSING get_order_items"
assert "get_order_payments" in WORKFLOW, "MISSING get_order_payments"
assert "get_payment_timeline" in WORKFLOW, "MISSING get_payment_timeline"
assert "get_refund_timeline" in WORKFLOW, "MISSING get_refund_timeline"
assert "get_shipment_summary" in WORKFLOW, "MISSING get_shipment_summary"
assert "get_sellers" in WORKFLOW, "MISSING get_sellers"
assert "get_policy" in WORKFLOW, "MISSING get_policy"
assert "get_item" not in WORKFLOW, "OLD name get_item still present"
assert '"get_payment"' not in WORKFLOW, "OLD name get_payment still present"
assert '"get_shipment"' not in WORKFLOW, "OLD name get_shipment still present"
print("All tool name assertions passed.")
